    ...
    async fn test_connection(&self) -> Result<ConnectionTestResult, ProviderError> {
        ...
        match timeout(Duration::from_secs(5), TcpStream::connect(addr)).await {
            Ok(Ok(_)) => Ok(ConnectionTestResult {
                success: true,
                message: format!(
                    "TCP connection to {} successful (workspace={})",
                    addr, self.workspace
                ),
...
impl SaasProvider for SlackProvider {
    ...
    async fn create(
        ...
        Ok(SaasResult {
            ...
            records_affected: 1,
            data: serde_json::json!({"type": object_type, "input": data, "channel": self.default_channel}),
            execution_time_ms: 0,
            rate_limit: None,
            next_cursor: None,
        })
    ...
    async fn update(
        ...
        Ok(SaasResult {
            ...
            records_affected: 1,
            data: serde_json::json!({"type": object_type, "ts": record_id, "input": data}),
            execution_time_ms: 0,
            rate_limit: None,
            next_cursor: None,
        })
    ...
    async fn delete(
        ...
        Ok(SaasResult {
            ...
            records_affected: 1,
            data: serde_json::json!({"type": object_type, "ts": record_id}),
            execution_time_ms: 0,
            rate_limit: None,
            next_cursor: None,
        })
