...
impl HubSpotProvider {
    pub fn from_config(name: &str, config: &ConnectionConfig) -> Result<Self, ProviderError> {
        ...
        Ok(Self { name: name.to_string(), portal_id })
    }
}
...
impl Provider for HubSpotProvider {
    ...
    async fn test_connection(&self) -> Result<ConnectionTestResult, ProviderError> {
        Ok(ConnectionTestResult {
            ...
            latency_ms: 0, server_version: Some("v3".to_string()),
        })
    }
...
impl SaasProvider for HubSpotProvider {
    ...
    async fn list_object_types(&self) -> Result<Vec<String>, ProviderError> {
        Ok(vec!["contacts".into(), "companies".into(), "deals".into(), "tickets".into(), "products".into(), "line_items".into()])
    }
}
