//! CockroachDB provider — uses CockroachDB wire protocol.
//!
//! Connects to CockroachDB clusters.
//!
//! # Configuration
//! ```yaml
//! connections:
...
//!     port: 26257
//!     database: defaultdb
//!     credentials: ${COCKROACH_PASSWORD}
//!     schema: public
//!     user: conduit
...
impl CockroachDbProvider {
    pub fn from_config(name: &str, config: &ConnectionConfig) -> Result<Self, ProviderError> {
        ...
        let database = config
            .database
            .clone()
            .unwrap_or_else(|| "defaultdb".to_string());
...
fn crdb_value_to_json(row: &sqlx::postgres::PgRow, idx: usize) -> serde_json::Value {
...
impl SqlProvider for CockroachDbProvider {
    async fn execute(
        ...
        let (query, bind_values) = super::params::bind_named_params(
            &query,
            params,
            super::params::PlaceholderStyle::Dollar,
        )
        ...
        if is_select {
            ...
            let sample_rows: Vec<Vec<serde_json::Value>> = rows[..sample_limit]
                ...
                .map(|row| {
                    (0..row.columns().len())
                        .map(|idx| crdb_value_to_json(row, idx))
                        .collect()
                })
                .collect();
