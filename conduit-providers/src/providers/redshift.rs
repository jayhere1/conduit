//! Amazon Redshift provider — uses Redshift wire protocol.
//!
...
impl RedshiftProvider {
    pub fn from_config(name: &str, config: &ConnectionConfig) -> Result<Self, ProviderError> {
        ...
        let ssl_mode = extra_str(config, "ssl_mode").unwrap_or_else(|| "require".to_string());
...
fn rs_value_to_json(row: &sqlx::postgres::PgRow, idx: usize) -> serde_json::Value {
...
impl Provider for RedshiftProvider {
    fn info(&self) -> ProviderInfo {
        ProviderInfo {
            ...
            display_name: format!("Redshift ({}:{})", self.host, self.port),
            version: None,
            capabilities: vec![
                Capability::SqlQuery,
                Capability::SqlDdl,
    ...
    async fn execute(
        ...
        if is_select {
            ...
            let sample_rows: Vec<Vec<serde_json::Value>> = rows[..sample_limit]
                ...
                .map(|row| {
                    (0..row.columns().len())
                        .map(|idx| rs_value_to_json(row, idx))
                        .collect()
                })
                .collect();
