...
| Stretch | Impact analysis as a GitHub Action; dbt-aware template resolution; dialect coverage | Each is small + high-evangelism | Impact GH Action ✓ (Bet 7); dialect-aware parsing ✓ (`7228999`); dbt template pending |
...
- **Lineage:** OpenLineage emit + ingest + Marquez round-trip + cross-task (Python→SQL→Python) lineage + dialect-aware parsing all shipped. Next compounding bet here is dbt template semantic awareness (lineage stops at the template boundary today — `ref()` / `source()` need manifest-driven resolution).
