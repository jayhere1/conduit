...
#### 3. Jinja/template *semantic* awareness — **shipped** (`1ad246b`)
Render-then-parse path landed: `DbtManifest` resolves `{{ ref(...) }}` and `{{ source(...) }}` to qualified table identifiers before `sqlparser` sees them. Macro / variable / config block resolution (`{{ this }}`, `{{ var(...) }}`) still falls through to placeholder — those need the dbt graph state, not just the manifest. Next slice on this axis is auto-loading the manifest in `cross_task::stitch` from a DAG-level config knob saying "this DAG is a dbt project rooted at X".
