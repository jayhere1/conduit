    ...
    if let Some(tc) = &task.contracts {
        for check in &tc.checks {
            ...
            if let Some(name) = metric_name {
                ...
                let history: Vec<serde_json::Value> = demo_values.iter().enumerate().map(|(i, v)| json!({
                    "day": format!("day-{}", 7 - i),
                    "value": v,
                })).collect();
                let trend = compute_trend(&demo_values);
                let min_val = demo_values.iter().cloned().fold(f64::INFINITY, f64::min);
                let max_val = demo_values.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
                let avg_val = demo_values.iter().sum::<f64>() / demo_values.len() as f64;
                let variance = demo_values.iter().map(|v| (v - avg_val).powi(2)).sum::<f64>() / demo_values.len() as f64;
...
fn generate_demo_values(metric_name: &str, dag_id: &str, task_id: &str) -> Vec<f64> {
    ...
    let seed: u64 = metric_name.bytes().chain(dag_id.bytes()).chain(task_id.bytes())
    ...
    match metric_name {
        ...
        "duplicate_count" => {
            (0..7).map(|i| {
                let noise = ((seed.wrapping_add(i * 557)) % 10) as f64;
                noise // 0-9 duplicates
            }).collect()
        }
        "row_count_delta_pct" => {
            (0..7).map(|i| {
        ...
        "enrichment_rate" | "currency_conversion_rate" | "session_attribution_rate" => {
            ...
            }).map(|v| v.min(1.0)).collect()
        }
        "referential_integrity_score" | "customer_match_rate" => {
            (0..7).map(|i| {
        ...
        "holdout_auc" | "training_auc" => {
            let base = if metric_name == "training_auc" { 0.82 } else { 0.78 };
            (0..7).map(|i| {
        ...
        "psi_max" | "concept_drift_score" | "covariate_drift_score" => {
            (0..7).map(|i| {
                let noise = ((seed.wrapping_add(i * 881)) % 100) as f64 / 1000.0;
                0.02 + noise
            }).collect()
        }
        "demographic_parity_delta" | "equalized_odds_delta" => {
            (0..7).map(|i| {
        ...
        "logo_churn_rate" | "revenue_churn_rate" => {
            let base = if metric_name == "logo_churn_rate" { 0.045 } else { 0.035 };
            (0..7).map(|i| {
